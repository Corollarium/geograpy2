...
class PlaceContext(object):
    ...
    def set_regions(self):
        ...
        for country in self.countries:
            ...
            matched_regions = [p for p in self.names if is_region(p, region_names)]
    ...
    def set_cities(self):
        ...
        cur = self.conn.cursor()
        cur.execute("SELECT * FROM cities WHERE city_name IN (" + ",".join("?"*len(self.names)) + ")", self.names)
        rows = cur.fetchall()
        ...
        all_cities = [p for p in self.names if p in self.cities]
    ...
    def set_other(self):
        ...
        self.other = [p for p in self.names if unused(p)]
